...
from utils.db_queries import (
    ...
    get_player_shooting_actuals, get_player_possession_actuals, get_player_transition_actuals
)
...
def style_diff_by_percent(row):
    ...
    for i, col_name in enumerate(row.index):
        if col_name.endswith('_Diff'):
            ...
            if percent_diff > 0: # Sim is higher (green)
                norm_val = min(percent_diff, PERCENT_CAP) / PERCENT_CAP
                color_map_val = 0.3 + (0.7 * norm_val)
                rgba_color = green_cmap(color_map_val)
            elif percent_diff < 0: # Sim is lower (red)
...
def render_shooting_validation_tab(results):
    if not results:
        ...
        return
    st.header("Shooting Performance (5v5): Simulation vs. Actuals (Per 60 Mins)")
    home_sim_df = results['home_players'].copy()
    away_sim_df = results['away_players'].copy()
    ...
    st.markdown("This shows the average difference between the simulation and actuals across all skaters in the game.")
...
def render_possession_validation_tab(results):
    ...
    st.header("Possession & Playmaking (5v5): Simulation vs. Actuals (Per 60 Mins)")
    home_sim_df = results['home_players'].copy()
    away_sim_df = results['away_players'].copy()
    all_player_ids = pd.concat([home_sim_df['player_id'], away_sim_df['player_id']]).unique().tolist()
...
def render_transition_validation_tab(results):
    ...
    st.header("Transition (5v5): Simulation vs. Actuals")
    home_sim_df = results['home_players'].copy()
    away_sim_df = results['away_players'].copy()
    all_player_ids = pd.concat([home_sim_df['player_id'], away_sim_df['player_id']]).unique().tolist()
    ...
    METRIC_MAP = [
        ...
        ('Sim_EntryDenial_Pct', 'actual_entry_denials_per_60', 'Entry Denials/60'),
    ]
    ...
    def process_transition_data(sim_df, actuals_df):
        ...
        attempts_against = sim_df.get('Sim_OnIce_EntryAttempts_Against_per_60_ES', 0)
        controlled_against = sim_df.get('Sim_OnIce_ControlledEntries_Against_per_60_ES', 0)
        sim_df['Sim_EntryDenial_Pct'] = np.where(
            attempts_against > 0, 
            attempts_against - controlled_against, # This is now a rate of denials per 60
            0
        )
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["Lineups", "Output", "per/60", "Player Level", "Shooting", "Possession", "Transition"])
    ...
    with tab4:
        ...
        if results_for_current_game:
            ...
            default_selection = [ 
                'Sim_iHDCF_per_60_ES', 'Sim_xG_for_per_60_ES', 'Sim_ControlledEntries_per_60_ES',
                'Sim_Giveaways_per_60_ES', 'Sim_Faceoff_Pct'
            ]
