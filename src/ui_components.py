# src/ui_components.py (Final Corrected Version)
...
def load_team_data(team_id: int, team_type: str, teams_df: pd.DataFrame):
    """
    Loads all data for a given team_id into the session state.
    """
    ...
    # --- FIX: Rename goalie 'full_name' to 'name' for consistency ---
    if 'full_name' in goalie_roster_df.columns:
        goalie_roster_df.rename(columns={'full_name': 'name'}, inplace=True)
    ...
    default_lineup = get_default_lineup(team_id)
    if 'full_name' in default_lineup.columns:
        default_lineup.rename(columns={'full_name': 'name'}, inplace=True)
        
    default_pp = get_default_pp_lineup(team_id)
    if 'full_name' in default_pp.columns:
        default_pp.rename(columns={'full_name': 'name'}, inplace=True)
        
    default_pk = get_default_pk_lineup(team_id)
    if 'full_name' in default_pk.columns:
        default_pk.rename(columns={'full_name': 'name'}, inplace=True)

    st.session_state.dashboard_data[team_type].update({
        'team_id': team_id, 'team_name': team_name,
        'lineup': default_lineup, 'pp_lineup': default_pp, 'pk_lineup': default_pk,
        'roster': skater_roster,
...
def _apply_saved_state(team_type: str, saved_state: dict):
    ...
    if saved_state.get(goalie_key) and saved_state[goalie_key] is not None:
        goalie_data = json.loads(saved_state[goalie_key])
        # --- FIX: Ensure consistency when loading saved goalie state ---
        if 'full_name' in goalie_data:
            goalie_data['name'] = goalie_data.pop('full_name')
        st.session_state.dashboard_data[team_type]['starting_goalie'] = goalie_data
...
def _render_ratings_editor(team_type: str, team_data: dict):
    ...
    SKATER_RATINGS_TO_EDIT = [ "toi_individual_rating", "faceoff_rating", "shooting_volume", "shooting_accuracy", "hdshot_creation", "mshot_creation", "ofinishing", "orebound_creation", "oprime_playmaking", "osecond_playmaking", "ozone_entry", "opuck_possession", "ocycle_play", "o_forechecking_pressure", "d_breakout_ability", "d_entry_denial", "d_cycle_defense", "d_shot_blocking", "o_hd_shot_creation_rating", "o_md_shot_creation_rating", "o_ld_shot_creation_rating", "d_hd_shot_suppression_rating", "d_md_shot_suppression_rating", "d_ld_shot_suppression_rating", "openalty_drawn", "min_penalty", "maj_penalty", "pp_shot_volume", "pp_shot_on_net", "pp_chance_creation", "pp_playmaking", "pp_zone_entry", "pp_finishing", "pp_rebound_creation", "pk_shot_suppression", "pk_clearing_ability", "pk_shot_blocking" ]
    GOALIE_RATINGS_TO_EDIT = [ "goalie_save_adj", "rebound_control_adj", "g_ld_save_5v5", "g_ld_save_4v5", "g_md_save_5v5", "g_md_save_4v5", "g_hd_save_5v5", "g_hd_save_4v5", "g_low_danger_sv_rating", "g_medium_danger_sv_rating", "g_high_danger_sv_rating", "g_rebound_control_rating", "g_freeze_puck_rating" ]
    ...
    with st.expander(f"✏️ Editing Ratings for: **{player_name}**", expanded=True):
        if not base_ratings_df.empty:
            ...
            h_col1.markdown("**Rating**"); h_col2.markdown("**Base**"); h_col3.markdown("**Manual**"); h_col4.markdown("**Manual Weight**")
            ...
            with save_col:
                if st.button("Save Ratings", key=f"save_expander_{team_type}_{player_id}", use_container_width=True, type="primary"):
                    PP_RATINGS = [ "pp_shot_volume", "pp_shot_on_net", "pp_chance_creation", "pp_playmaking", "pp_zone_entry", "pp_finishing", "pp_rebound_creation" ]
                    PK_RATINGS = [ "pk_shot_suppression", "pk_clearing_ability", "pk_shot_blocking" ]
                    edited_base_ratings = {}; edited_pp_ratings = {}; edited_pk_ratings = {}
                    ...
                    if edited_base_ratings and not is_goalie: update_base_rating(player_id, edited_base_ratings)
                    if edited_pp_ratings and not is_goalie: update_pp_rating(player_id, edited_pp_ratings)
                    if edited_pk_ratings and not is_goalie: update_pk_rating(player_id, edited_pk_ratings)
...
def _render_lineup_rows(team_type, line_name, positions, team_data, player_names, toi_component: str = 'Total'):
    def update_player_in_lineup(pos_lookup_val):
        ...
        if "PP" in line_name: lineup_df_key, pos_col = 'pp_lineup', 'pp_position'
        elif "PK" in line_name: lineup_df_key, pos_col = 'pk_lineup', 'pk_position'
        else: lineup_df_key, pos_col = 'lineup', 'position_slot'
        ...
        if selected_player_name:
            new_player_info = roster_df[roster_df['name'] == selected_player_name]
            if not new_player_info.empty:
                new_player_id = new_player_info['player_id'].iloc[0]
                new_player_pos = new_player_info['position'].iloc[0]
                if row_index:
                    idx = row_index[0]
                    st.session_state.dashboard_data[team_type][lineup_df_key].loc[idx, 'name'] = selected_player_name
                    ...
                    new_row = pd.DataFrame([{'name': selected_player_name, 'player_id': new_player_id, 'position': new_player_pos, pos_col: pos_lookup_val}])
                    st.session_state.dashboard_data[team_type][lineup_df_key] = pd.concat([lineup_df, new_row], ignore_index=True)
        elif row_index:
            idx = row_index[0]
            st.session_state.dashboard_data[team_type][lineup_df_key].loc[idx, ['name', 'player_id', 'position']] = [None, None, None]
    
    if "PP" in line_name: lineup_df, pos_col = team_data.get('pp_lineup', pd.DataFrame()), 'pp_position'
    elif "PK" in line_name: lineup_df, pos_col = team_data.get('pk_lineup', pd.DataFrame()), 'pk_position'
    else: lineup_df, pos_col = team_data.get('lineup', pd.DataFrame()), 'position_slot'
    ...
    def render_player(pos_lookup_val):
        ...
        if not lineup_df.empty and pos_col in lineup_df.columns:
            ...
            if not player_row.empty and pd.notna(player_row['name'].iloc[0]):
                default_player_name = player_row['name'].iloc[0]
        ...
        with id_col:
            ...
            st.button(label=str(default_player_id), key=f"{team_type}_{default_player_id}_{pos_lookup_val}_select", on_click=select_player, args=(team_type, str(default_player_id), default_player_name), disabled=(str(default_player_id) == "N/A"), use_container_width=True, type=("primary" if is_selected else "secondary"))
        with name_col:
            st.selectbox(label=''.join(c for c in pos_lookup_val if c.isalpha()), options=roster_options, index=default_index, key=f"{team_type}_{line_name}_{pos_lookup_val}_name", label_visibility="collapsed", on_change=update_player_in_lineup, args=(pos_lookup_val,))
...
def _render_goalie_ui(team_type: str, team_data: dict):
    ...
    if goalie_roster.empty:
        ...
        return
    goalie_names = goalie_roster['name'].tolist()
    current_goalie = team_data.get('starting_goalie')
    try:
        current_index = goalie_names.index(current_goalie['name']) if current_goalie and 'name' in current_goalie else 0
    except (ValueError, TypeError, KeyError):
        current_index = 0
    
    def on_goalie_change():
        selected_name = st.session_state[f"{team_type}_goalie_selector"]
        selected_goalie_row = goalie_roster[goalie_roster['name'] == selected_name]
    ...
    with id_col:
        ...
        st.button(label=str(goalie_id), key=f"{team_type}_{goalie_id}_goalie_select", on_click=select_player, args=(team_type, str(goalie_id), current_goalie.get('name', "N/A") if current_goalie else "N/A"), disabled=(str(goalie_id) == "N/A"), use_container_width=True, type=("primary" if is_selected else "secondary"))
    with name_col:
        st.selectbox(label="GoalieSelect", options=goalie_names, index=current_index, key=f"{team_type}_goalie_selector", on_change=on_goalie_change, label_visibility="collapsed")
...
def _render_coach_editor(team_type: str, team_data: dict):
    ...
    with st.expander(f"⚙️ Editing Ratings for Coach: **{coach_name}**", expanded=True):
        ...
        new_pp1_percent = st.slider("PP1 Share (%)", min_value=0, max_value=100, value=pp1_share_percent, key=f"{team_type}_pp_share_slider")
        ...
        new_pk1_percent = st.slider("PK1 Share (%)", min_value=0, max_value=100, value=pk1_share_percent, key=f"{team_type}_pk_share_slider")
        ...
        with save_col:
            if st.button("Save Coach Ratings", key=f"save_coach_{team_type}", use_container_width=True, type="primary"):
                new_pp_unit_shares = { "PP1": new_pp1_percent / 100.0, "PP2": pp2_share_percent / 100.0 }
                new_pk_unit_shares = { "PK1": new_pk1_percent / 100.0, "PK2": pk2_share_percent / 100.0 }
                
                payload = { 'pp_unit_shares': new_pp_unit_shares, 'pk_unit_shares': new_pk_unit_shares }
...
def render_team_ui(team_type: str, teams_df: pd.DataFrame):
    ...
    if is_team_selected:
        ...
        if coach_info:
            ...
            with id_col:
                st.button(label=str(coach_info.get('coach_id', 'N/A')), key=f"edit_coach_{team_type}", on_click=toggle_coach_editor, args=(team_type,), use_container_width=True)
            with name_col:
                st.text_input("Coach", value=coach_info.get('coach', 'N/A'), disabled=True, key=f"{team_type}_coach_name", label_visibility="collapsed")
    ...
    for i, pos in enumerate(positions):
        with line_cols[i]:
            ...
            if not lineup_df.empty and position_col_name in lineup_df.columns:
                ...
                if not default_player.empty and pd.notna(default_player['name'].iloc[0]):
                    default_name = default_player['name'].iloc[0]
                    try:
                        default_index = roster_names.index(default_name)
                    except (ValueError, TypeError):
                        default_index = None
            selected_player_name = st.selectbox(label=pos, options=roster_names, index=default_index, placeholder="Select Player...", key=widget_key)
            if selected_player_name:
                player_id = roster_df[roster_df['name'] == selected_player_name]['player_id'].iloc[0]
